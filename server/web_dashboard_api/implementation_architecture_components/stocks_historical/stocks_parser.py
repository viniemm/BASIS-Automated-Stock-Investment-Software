...
from web_dashboard_api.implementation_architecture_components.stocks_historical.stocks_derived_model import \
    StocksDerivedModel
from web_dashboard_api.models import StocksData


class StocksDerivedModelParser(BaseDerivedModelParser):
    ...
    def apply_complex_filter(self, complex_filter: QueryComplexFilter) -> QuerySet:
        filtered = StocksData.objects.complex_filter(self.complex_filter_to_query(complex_filter))
        # Adding time filter
    ...
    def get_derived_model_class(self) -> typing.Type[StocksDerivedModel]:
        return StocksDerivedModel

    def get_queryset_value_list(self) -> [str]:
        values_list = []
        values_list.append("date")
        values_list.append("symbol")
        values_list.append("close")
